...
{
    public class GeneratorOracleCreate : GeneratorDb
    {

        public override string DisplayName
        {
            get { return "Oracle : Création"; }
        }
        ...
        public override void GenerateUser(ScriptWriter writer, Revision sourceRevision, Revision targetRevision)
        {
            ...
            foreach (DbSchema targetSchema in targetRevision.ResultSchemas)
            {
                    ...
                    writer.WriteLine("CREATE USER " + targetSchema.Name + "_user_consult IDENTIFIED BY " + targetSchema.Name + "_user_consult DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;");
                    writer.WriteLine("CREATE USER " + targetSchema.Name + "_user_admin IDENTIFIED BY " + targetSchema.Name + "_user_admin DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;");
        ...
        public override void GenerateSchema(ScriptWriter writer, Revision sourceRevision, Revision targetRevision)
        {
            ...
            foreach (DbSchema targetSchema in targetRevision.ResultSchemas)
            {
                if (!(from s in sourceRevision.ResultSchemas where s.Name.Equals(targetSchema.Name) select s).Any())
                {
                    ...
                    writer.WriteLine("CREATE USER " + targetSchema.Name + " IDENTIFIED BY " + targetSchema.Name + ";");
                }
            }
            writer.WriteLine("");
        }
        ...
        public override void GenerateGrant(ScriptWriter writer, Revision sourceRevision, Revision targetRevision)
        {
            ...
            foreach (DbSchema targetSchema in targetRevision.ResultSchemas)
            {
                ...
                foreach (DbSchema targetSchemaIter in targetRevision.ResultSchemas)
                {
                    foreach (DbTable targetTable in targetSchema.Tables)
                    {
                        DbColumnId dbColumnId = (from c in targetTable.Columns where c is DbColumnId select c as DbColumnId).FirstOrDefault();
                        if (dbColumnId != null)
                        {
                            writer.WriteComment("Droit des utilisateurs du schéma " + targetSchema.Name + " sur la sequence " + targetTable.Name + "_id_seq");
                            writer.WriteLine("GRANT SELECT ON " + targetSchema.Name + "." + targetTable.Name + "_id_seq TO " + targetSchema.Name + "_role_consult;");
                            writer.WriteLine("");
                        }
                    }
                }
                ...
                writer.WriteComment("Attribution des droits de connexion");
                writer.WriteLine("GRANT CONNECT,CREATE SESSION TO " + targetSchema.Name + ";");
                writer.WriteLine("GRANT CONNECT,CREATE SESSION TO " + targetSchema.Name + "_user_consult;");
                writer.WriteLine("GRANT CONNECT,CREATE SESSION TO " + targetSchema.Name + "_user_admin;");        
        ...
        public override void GenerateSequence(ScriptWriter writer, Revision sourceRevision, Revision targetRevision)
        {
            base.GenerateSequence(writer, sourceRevision, targetRevision);
            foreach (DbSchema targetSchema in targetRevision.ResultSchemas)
            {
                    ...
                    foreach (DbTable targetTable in targetSchema.Tables)
                    {
                        ...
                        if (dbColumnId != null)
                        {
                            writer.WriteComment("Création de la sequence " + targetTable.Name + "_id_seq pour la table " + targetTable.Name);
                            writer.WriteLine("CREATE SEQUENCE " + targetSchema.Name + "." + targetTable.Name + "_id_seq;");
                            writer.WriteLine("");
                        }
                    }
        ...
        public override void GenerateTrigger(ScriptWriter writer, Revision sourceRevision, Revision targetRevision)
        {
            base.GenerateTrigger(writer, sourceRevision, targetRevision);
            foreach (DbSchema targetSchema in targetRevision.ResultSchemas)
            {
                ...
                foreach (DbTable targetTable in targetSchema.Tables)
                {
                    ...
                    if (dbColumnId != null)
                    {
                        writer.WriteComment("Création du trigger de la sequence " + targetTable.Name + "_id_seq pour la table " + targetTable.Name);
                        writer.WriteLine("CREATE OR REPLACE TRIGGER " + targetSchema.Name + ".tib_" + targetTable.Name);
                        writer.WriteLine("BEFORE INSERT ON " + targetSchema.Name + "." + targetTable.Name + "");
                        writer.WriteLine("FOR EACH ROW");
                        writer.WriteLine("DECLARE");
                        writer.WriteLine("BEGIN");
                        writer.WriteLine("  IF(:new." + dbColumnId.Name + " IS NULL)");
                        writer.WriteLine("  THEN");
                        writer.WriteLine("    :new.id := " + targetTable.Name + "_id_seq.nextval;");
        ...
        private string GetColumnDefinition(DbColumn dbColumn)
        {
            ...
            {
                return "BLOB" + notNullString;
            }
            else if (dbColumn is DbColumnGeom)
            {
                return "CLOB" + notNullString;
            }
            else if (dbColumn is DbColumnId)
            {
                return "NUMBER(8)" + notNullString;
            }
            ...
            else if (dbColumn is DbColumnText)
            {
                ...
                return "VARCHAR2(" + dbColumnText.MaxLength + " CHAR)" + notNullString;
            }
            else
            {
